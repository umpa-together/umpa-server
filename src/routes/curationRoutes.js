...
router.get('/curationposts/:page', async (req, res) => {
    const curationposts = await Curationpost.find({$or: [{ $and: [{  postUserId:{$in:req.user.following}}, {hidden:false} ]}, {postUserId: req.user._id}]} ).sort({'time': -1}).populate('postUserId').skip(req.params.page*20).limit(20);
    res.send(curationposts);
})
