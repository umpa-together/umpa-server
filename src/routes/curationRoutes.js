...
router.get('/curationposts', async (req,res) => {
    const curationposts = await Curationpost.find({$or: [{ $and: [{  postUserId:{$in:req.user.following}}, {hidden:false} ]}, {postUesrId: req.user_id}]} ).sort({'time': -1}).populate('postUserId').limit(20);
    res.send(curationposts);
});

router.get('/curationposts/:page', async (req, res) => {
    const curationposts = await Curationpost.find({$or: [{ $and: [{  postUserId:{$in:req.user.following}}, {hidden:false} ]}, {postUesrId: req.user_id}]} ).sort({'time': -1}).populate('postUserId').skip(req.params.page*20).limit(20);
    res.send(curationposts);
})
